        ...
        "columns": [
            ...
            { "data": "nombre_completo" },
            {
                ...
                "render": function (data, type, row) {
                    ...
                    if (modds.includes('C')) { 
                    buttons += `<li><a class="dropdown-item" id="btn-social" data-id="${data.id}" data-bs-toggle="modal" data-bs-target="#ModalSocial" href="#" ><i class="fa-solid fa-users"></i> Datos Sociales</a></li>`; 
                    }
                    if (modcf.includes('C')) { 
                    buttons += `<li><a class="dropdown-item" id="btn-dependientes" data-id="${data.id}" data-bs-toggle="modal" data-bs-target="#ModalDependientes" href="#"><i class="fa-solid fa-people-roof"></i> Carga Familiar</a></li>`;
                    } 
                    if (modhd.includes('C')) { 
                    buttons += `<li><a class="dropdown-item" id="btn-historial" data-id="${data.id}" data-bs-toggle="modal" data-bs-target="#ModalHistorial" href="#"><i class="fa-solid fa-medal"></i> Historial Deportivo</a></li>`; 
                    }
                    if (modbf.includes('C')) { 
                    buttons += `<li><a class="dropdown-item" id="btn-beneficios" data-id="${data.id}" data-bs-toggle="modal" data-bs-target="#ModalBeneficios" href="#"><i class="fa-regular fa-money-bill-1"></i> Beneficios Asignados</a></li>`; 
                    }
            ...
            },
            {
                ...
                "render": function (data, type, row) { 
                    let buttons = `<div class="btn-container">`; 
                      if (modgl.includes('R')) { 
                        buttons += `<button type="button" id="btn-view" class="btn-sm btn-info btn btn3d" data-id="${data.id}" data-bs-toggle="modal" data-bs-target="#staticBackdrop"><i class="fa-regular fa-eye"></i></button>`; 
                      }
                      if (modgl.includes('U')) { 
                        buttons += `<button type="button" id="btn-edit" class="btn-sm btn-warning btn btn3d" data-id="${data.id}" data-bs-toggle="modal" data-bs-target="#staticBackdrop"><i class="fa-duotone fa-solid fa-pen-to-square"></i></button>`;
                      } 
                      if (modgl.includes('D')) { 
                        buttons += `<button type="button" id="btn-delete" class="btn-sm btn-danger btn btn3d" data-id="${data.id}"><i class="fa-duotone fa-solid fa-trash"></i></button>`; 
                    }
                     buttons += `</div>`; 
                     return buttons; 
                }
